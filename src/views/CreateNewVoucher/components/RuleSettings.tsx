...
const RuleSettings = () => {
    ...
    const [orices, setOrices] = useState('')
                ...
                rules={[
                    {
                        validator: (_, value) => {
                            ...
                            return value < 100000 ? Promise.resolve() : Promise.reject(new Error('Please note that the discount amount is > 60% of min basket price'))
                        }
                    },
                ]}
            >
                        ...
                        <div className="flex items-center border border-gray-300 border-solid">
                            ...
                            {DiscountType !== 'By Percentage' ? (
                                <>
                                    ...
                                    <Form.Item name="Amount" className="m-0 flex-1 h-8" rules={[
                                        {
                                            validator: (_, value) => {
                                                return value < 100000 ? Promise.resolve() : Promise.reject(new Error('Please note that the discount amount is > 60% of min basket price'))
                                            },
                                        ...
                                        {
                                            validator: (_, value) => {
                                                return (value > 1) && (value < 99) ? Promise.resolve() : Promise.reject(new Error('Please enter a value between 1 and 99'))
                                            }
                                        }
                                    ]}>
                                ...
                                <>
                                    <Form.Item
                                        name="Amount"
                                        className="m-0 flex-1 h-8"
                                        required
                                    >
                                        <InputNumber controls={false} placeholder='Input' className="border-0 border-l rounded-none w-full" />
                                    </Form.Item>
