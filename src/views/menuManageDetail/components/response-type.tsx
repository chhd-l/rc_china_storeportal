    ...
    <div className='response-type-container bg-white border border-gray-200'>
      <div className='p-2 bg-gray-300 text-gray-400 flex space-x-8'>
        <div
          ...
          <span>{intl.get('wx.graphic_message')}</span>
        </div>
      ...
      <div className='p-8 text-center'>
        <div className='flex justify-center items-start'>
          <div className='mx-md text-gray-400' onClick={() => setVisible(true)}>
            ...
            <div>{intl.get('wx.select_from_assets')}</div>
          </div>
        ...
        <div className='mt-4'>
          {activeMenu?.rc_preview_type === "news" && activeMenu?.rc_preview_news_media_id ? <div className='uploaded-asset'>{intl.get('wx.assets_id')}: {activeMenu?.rc_preview_news_media_id}</div> : null}
          {activeMenu?.rc_preview_type === "image" && activeMenu?.rc_preview_image_media_id ? <div className='uploaded-asset'>{intl.get('wx.assets_id')}: {activeMenu?.rc_preview_image_media_id}</div> : null}
          {activeMenu?.rc_preview_type === "voice" && activeMenu?.rc_preview_voice_media_id ? <div className='uploaded-asset'>{intl.get('wx.assets_id')}: {activeMenu?.rc_preview_voice_media_id}</div> : null}
          {activeMenu?.rc_preview_type === "video" && activeMenu?.rc_preview_video_media_id ? <div className='uploaded-asset'>{intl.get('wx.assets_id')}: {activeMenu?.rc_preview_video_media_id}</div> : null}
