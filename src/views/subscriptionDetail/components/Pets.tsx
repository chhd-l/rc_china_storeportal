...
const calcPetAge = (dateStr: string) => {
  ...
  if (diffMonth <= 1) {
    ...
    return `${diffYear} ${diffYear > 1 ? 'years' : 'year'} ${
      diffMonthAfterYear === 0 ? '' : `${diffMonthAfterYear} ${diffMonthAfterYear > 1 ? 'months' : 'month'}`
    }`
  }
}
  ...
  return (
    <div>
      <div className="flex justify-start space-x-4">
        ...
        <span>{intl.get('subscription.Pet Detail')}</span>
      </div>
      <div className="mt-4 flex items-center space-x-4">
        <div>
          <Avatar shape="square" size={64} src={pet?.image} />
        </div>
        <div className="flex-grow">
          <div>{pet?.name}</div>
          <Row gutter={10}>
            <Col span={4}>
              <span className="text-gray-400">{intl.get('subscription.Age')}</span>
            </Col>
            <Col span={4}>
              <span className="text-gray-400">{intl.get('subscription.Breed')}</span>
            </Col>
          </Row>
          <Row gutter={10}>
