          ...
          {(data?.productList ?? []).map((product: any, idx: number) => (
            <Row key={idx}>
              ...
              <Col span={16}>
                <Row
                  className={`${
                    (data?.productList ?? []).length > 1 && idx < (data?.productList ?? []).length - 1
                      ? 'border-b h-20 pb-2'
                      : ''
                  }`}
                >
                  <Col span={20}>
                    ...
                    <span className="text-gray-400 text-sm">
                      {intl.get('subscription.Variation')}:{' '}
                      {(product?.specifications ?? [])
                        .map((vari: any) =>
                          (vari?.specificationDetails ?? [])
                            .map((variitem: any) => variitem.specificationDetailNameEn)
                            .join(','),
                        )
                        .join(',')}
                    </span>
        ...
        <Col span={2} className="text-left">
          <div className="space-x-2">
            <Tooltip title="View Details">
              <Link
                to="/subscription/subscription-detail"
                state={{ id: data?.id }}
                className="cursor-pointer iconfont icon-kjafg primary-color"
              />
            </Tooltip>
            {data?.status === 'PAUSED' || data?.status === 'ONGOING' ? (
              <Tooltip title={data?.status === 'PAUSED' ? 'Restart' : 'Pause'}>
                <span
                  className={`cursor-pointer iconfont primary-color ${
                    data?.status === 'PAUSED' ? 'icon-Vector2' : 'icon-Vector-1'
                  }`}
