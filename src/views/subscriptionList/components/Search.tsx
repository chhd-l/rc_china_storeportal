  ...
  return (
    <div>
      <div className="flex flex-row items-center justify-end space-x-4">
        <div className="w-auto mr-3">{intl.get('subscription.Subscription Creation Date')}</div>
        <DatePicker.RangePicker style={{ width: '300px' }} value={dates} onChange={handleSelectDate} />
      ...
      <div className="flex flex-row items-center mt-4 text-left">
        <Input.Group compact>
          <Select
            ...
            style={{ width: '20%' }}
          >
            <Option value="noLike">{intl.get('subscription.Subscription ID')}</Option>
            {/* <Option value="type">Subscription Type</Option> */}
            <Option value="cycle">{intl.get('subscription.Subscription Cycle')}</Option>
            <Option value="orderIdLike">{intl.get('subscription.Order ID')}</Option>
            <Option value="phoneLike">{intl.get('subscription.Phone Number')}</Option>
            <Option value="wechatNameLike">{intl.get('subscription.Wechat Name')}</Option>
            <Option value="productNameLike">{intl.get('subscription.Product Name')}</Option>
          </Select>
          {type === 'type' ? (
            <Select
        ...
        <Button className="w-32 mx-3 btn-primary" type="primary" onClick={handleSearch}>
          {intl.get('public.search')}
        </Button>
        <Button className="w-32" onClick={handleReset}>
