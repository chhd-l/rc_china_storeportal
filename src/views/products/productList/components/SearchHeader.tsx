...
const SearchHeader = ({getFormData}:any) => {
  ...
  const nameForKey = [{ name: 'Product Name', value: 'ProductName' }, { name: 'SKU', value: 'SKU' }, { name: 'SPU', value: 'SPU' }]
  const typeForKey = [{ name: 'Product Type', value: 'ProductType' }, { name: 'Subscription Status', value: 'SubscriptionStatus' }]
  ...
  return (
    ...
    >
      {nameForKey.map((el: any) => <Option key={el.value} value={el.value}>{el.name}</Option>)}
        ...
        <Col span={12}>
          <Form.Item label="Category" name="username1">
            <Input placeholder="input placeholder" />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item label="Stock" name="username2">
            <Input placeholder="input placeholder" />
          </Form.Item>
        </Col>
        <Col span={12}>
            <Input.Group compact className="flex">
            <Form.Item name="username22" >
          <Select
      style={{ width: 140 }}
      placeholder="Select a option and change input text above"
      allowClear
